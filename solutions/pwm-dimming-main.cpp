...
ISR(TIMER1_COMPA_vect) {
  ...
  if (i_ramp >= MAX_BRIGHTNESS) {
    ...
    const uint8_t first_led_in_row_idx = i_row_idx << 1; // Multiply with 2
    i_row_outputs = (1 << i_row_idx); 
    i_row_mask = i_row_outputs - 1; // Set bits for all columns less than current row_idx
